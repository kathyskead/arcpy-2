     ...
     INPUTS:
      ...
      join_attributes {String}:
          ...
          * ONLY_FID—All the attributes from the input features but only the FID from the
          ...
          features FID attribute value in the output will be -1 if no intersection is
          found.
      cluster_tolerance {Linear unit}:
          The minimum distance separating all feature coordinates (nodes and vertices) as
          well as the distance a coordinate can move in X or Y (or both).
    ...
    """SpatialJoin_analysis(target_features, join_features, out_feature_class, {join_operation}, {join_type}, {field_mapping}, {match_option}, {search_radius}, {distance_field_name})
     ...
     INPUTS:
      target_features (Feature Layer):
          ...
          be defined in the field map parameter.
      join_features (Feature Layer):
          The attributes from the join features are joined to the attributes of the target
          features. See the explanation of the join_operation parameter for details on how
      ...
      join_type {Boolean}:
          ...
          * KEEP_COMMON— Only those target features that have the specified spatial
          ...
          with  match_option = "WITHIN", the output feature class will only contain those
      ...
      match_option {String}:
          ...
          * INTERSECT—The features in the join features will be matched if they intersect
          a target feature. This is the default. Specify a distance in the search_radius
          parameter.

          * INTERSECT_3D— The features in the join features will be matched if they
          intersect a target feature in three-dimensional space (x, y, and z). Specify a
          ...
          * WITHIN_A_DISTANCE_GEODESIC—Same as WITHIN_A_DISTANCE except that geodesic
          distance is used rather than planar distance. Choose this if your data covers a
          large geographic extent or the coordinate system of the inputs is unsuitable for
          ...
          * BOUNDARY_TOUCHES—The features in the join features will be matched if they
          have a boundary that touches a target feature. When the target and join features
          are lines or polygons, the boundary of the join feature can only touch the
          boundary of the target feature and no part of the join feature can cross the
          boundary of the target feature.
          ...
          * HAVE_THEIR_CENTER_IN—The features in the join features will be matched if a
          ...
          used, and for line input the geometry's midpoint is used. Specify a distance in
          the search_radius parameter.

          * CLOSEST—The feature in the join features that is closest to a target feature
          is matched. See the usage tip for more information. Specify a distance in the
          search_radius parameter.

          * CLOSEST_GEODESIC—Same as CLOSEST except that geodesic distance is used rather
          than planar distance. Choose this if your data covers a large geographic extent
          or the coordinate system of the inputs is unsuitable for distance calculations
      search_radius {Linear unit}:
          ...
          (Match Option) INTERSECT, WITHIN_A_DISTANCE, WITHIN_A_DISTANCE_GEODESIC,
          HAVE_THEIR_CENTER_IN, CLOSEST or CLOSEST_GEODESIC is specified. Using a search
          radius of 100 meters with the spatial relationship WITHIN_A_DISTANCE will join
          feature within 100 meters of a target feature. For the three WITHIN_A_DISTANCE
          relationships, if no value is specified for search radius then a distance of 0
          is used.
      distance_field_name {String}:
          ...
          only valid when the spatial relationship (Match Option) CLOSEST or
          CLOSEST_GEODESIC is specified. The value of this field is -1 if no feature is
          matched within a search radius. If no field name is specified, the field will
          not be added to the output feature class.
     ...
     INPUTS:
      ...
      gaps {Boolean}:
          ...
          * NO_GAPS—A feature will be created for the areas in the output that are
          completely enclosed by polygons. This feature will have blank attributes and its
          FID values will be -1.
...
def Buffer(in_features=None, out_feature_class=None, buffer_distance_or_field=None, line_side=None, line_end_type=None, dissolve_option=None, dissolve_field=None, method=None):
    """Buffer_analysis(in_features, out_feature_class, buffer_distance_or_field, {line_side}, {line_end_type}, {dissolve_option}, {dissolve_field;dissolve_field...}, {method})
     ...
     INPUTS:
      ...
      buffer_distance_or_field (Linear unit / Field):
          ...
          the input features' spatial reference is used.When specifying a distance, if the
          desired linear unit has two words, such as
      ...
      dissolve_field {Field}:
          The list of fields from the input features on which to dissolve the output
          buffers. Any buffers sharing attribute values in the listed fields (carried over
          from the input features) are dissolved.
      method {String}:
          Specifies what method to use, planar or geodesic, to create the buffer.

          * PLANAR—If the input features are in a projected coordinate system, Euclidean
          buffers are created. If the input features are in a geographic coordinate system
          and the buffer distance is in linear units (meters, feet, and so forth, as
          opposed to angular units such as degrees), geodesic buffers are created. This is
          the default. You can use the Output Coordinate System environment setting to
          specify the coordinate system to use. For example, if your input features are in
          a projected coordinate system, you can set the environment to a geographic
          coordinate system in order to create geodesic buffers.

          * GEODESIC—All buffers are created using a shape-preserving geodesic buffer
          method, regardless of input coordinate system.
...
def CreateThiessenPolygons(in_features=None, out_feature_class=None, fields_to_copy=None):
    """CreateThiessenPolygons_analysis(in_features, out_feature_class, {fields_to_copy})
        ...
        Creates Thiessen polygons from point features.Each Thiessen polygon contains
        only a single point input feature. Any location
     ...
     INPUTS:
      ...
      fields_to_copy {String}:
          Determines which fields from the input features will be transferred to the
          output feature class.

          * ONLY_FID—Only the FID field from the input features will be transferred to the
          output feature class. This is the default.

          * ALL—All fields from the input features will be transferred to the output
          feature class.
...
def GenerateNearTable(in_features=None, near_features=None, out_table=None, search_radius=None, location=None, angle=None, closest=None, closest_count=None, method=None):
    """GenerateNearTable_analysis(in_features, near_features;near_features..., out_table, {search_radius}, {location}, {angle}, {closest}, {closest_count}, {method})

        Calculates distances and other proximity information between features in one or
        more feature class or layer. Unlike the Near tool, which modifies the input,
        Generate Near Table writes results to a new stand-alone table and supports
        finding more than one near feature.

     INPUTS:
      in_features (Feature Layer):
          The input features that can be point, polyline, polygon, or multipoint type.
      near_features (Feature Layer):
          One or more layer of feature class containing near feature candidates. The near
          features can be of point, polyline, polygon, or multipoint. If multiple layers
          or feature classes is specified, a field named NEAR_FC is added to the input
          table and will store the paths of the source feature class containing the
          nearest feature found. The same feature class or layer may be used as both input
          and near features.
      search_radius {Linear unit}:
          The radius used to search for near features. If no value is specified, all near
          features will be candidates. If a distance is entered, but the unit is left
          blank or set to Unknown, the units of the coordinate system of the input
          features are used. If the Geodesic option is used in the Method parameter, a
          linear unit such as Kilometers or Miles should be used.
      location {Boolean}:
          Specifies whether x- and y-coordinates of the input feature's location and
          nearest location of the near feature will be written to the FROM_X, FROM_Y,
          NEAR_X, and NEAR_Y fields.

          * NO_LOCATION— Locations will not be written to the output table. This is the
          default.

          * LOCATION— Locations will be written to the output table.
      angle {Boolean}:
          Specifies whether the near angle will be calculated and written to a NEAR_ANGLE
          field in the output table. A near angle measures direction of the line
          connecting an input feature to its nearest feature at their closest locations.
          When the PLANAR method is used in the method parameter, the angle is within the
          range of -180° to 180°, with 0° to the east, 90° to the north, 180° (or -180°)
          to the west, and -90° to the south. When the GEODESIC method is used, the angle
          is within the range of -180° to 180°, with 0° to the north, 90° to the east,
          180° (or -180°) to the south, and -90° to the west.

          * NO_ANGLE—NEAR_ANGLE will not be added to the output table. This is the
          default.
      ...
      closest {Boolean}:
          ...
          * CLOSEST—Only the closest near feature will be written to the output table.
          This is the default.

          * ALL—Multiple near features will be written to the output table (a limit can be
          specified in the closest_count parameter).
      closest_count {Long}:
          Limit the number of near features reported for each input feature. This
          parameter is ignored if the closest parameter is set to CLOSEST.
      method {String}:
          Determines whether to use a shortest path on a spheroid (geodesic) or a flat
          earth (planar). It is strongly suggested to use Geodesic method with data stored
          in a coordinate system which is not appropriate for distance measurements (for
          example, Web Mercator and any geographic coordinate system), or any dataset
          which spans a large geographic area.

          * PLANAR—Uses planar distances between the features. This is the default.

          * GEODESIC—Uses geodesic distances between features. This method takes into
          account the curvature of the spheroid and correctly deals with data near the
          dateline and poles.
    ...
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject
    try:
        retval = convertArcObjectToPythonObject(gp.GenerateNearTable_analysis(*gp_fixargs((in_features, near_features, out_table, search_radius, location, angle, closest, closest_count, method), True)))
        return retval
    except Exception, e:
        raise e
...
def Near(in_features=None, near_features=None, search_radius=None, location=None, angle=None, method=None):
    """Near_analysis(in_features, near_features;near_features..., {search_radius}, {location}, {angle}, {method})

        Calculates distance and additional proximity information between the input
        features and the closest feature in another layer or feature class.
     ...
     INPUTS:
      ...
      near_features (Feature Layer):
          ...
          candidates. The near features can be of point, polyline, polygon, or multipoint.
          If multiple layers or feature classes are specified, a field named NEAR_FC is
          added to the input table and will store the paths of the source feature class
          containing the nearest feature found. The same feature class or layer may be
          used as both input and near features.
      search_radius {Linear unit}:
          The radius used to search for near features. If no value is specified, all near
          features are considered. If a distance but no unit or unknown is specified, the
          units of the coordinate system of the input features are used. If the Geodesic
          option is used, a linear unit such as Kilometers or Miles should be used.
      location {Boolean}:
          Specifies whether x- and y-coordinates of the closest location of the near
          feature will be written to the NEAR_X and NEAR_Y fields.

          * NO_LOCATION— Location information will not be written to the output table.
          This is the default.
      ...
      method {String}:
          Determines whether to use a shortest path on a spheroid (geodesic) or a flat
          earth (planar) method. It is strongly suggested to use the Geodesic method with
          data stored in a coordinate system that is not appropriate for distance
          measurements (for example, Web Mercator or any geographic coordinate system) and
          any analysis that spans a large geographic area.
...
def PointDistance(in_features=None, near_features=None, out_table=None, search_radius=None):
    """PointDistance_analysis(in_features, near_features, out_table, {search_radius})
        ...
        features within a specified search radius.This is a deprecated tool. This
        functionality has been replaced by Near and
        Generate Near Table tools that now calculate distances between point, polyline
    ...
    """PolygonNeighbors_analysis(in_features, out_table, {in_fields;in_fields...}, {area_overlap}, {both_sides}, {cluster_tolerance}, {out_linear_units}, {out_area_units})
     ...
     INPUTS:
      ...
      out_area_units {String}:
          ...
          the input feature units. This parameter is only enabled when area_overlap =
          "AREA_OVERLAP".
