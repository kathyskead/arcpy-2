...
class Layer(mixins.LayerIterationMixin,mixins.LayerMixin,mixins.LayerSupportsPropertyMixin,_BaseArcObject):
    ...
    def setSelectionSet(self, method, oidList):
        """Layer.setSelectionSet(method, oidList)
        ...
        return convertArcObjectToPythonObject(self._arc_object.setSelectionSet(*gp_fixargs((method, oidList), True)))
    
    def updateLayerFromJSON(self, json_layer_definition):
        """Layer.updateLayerFromJSON(json_layer_definition)
           
           Updates a Layer from a JSON string.
           
             json_layer_definition(String):
           The layer definition in JavaScript Object Notation (JSON) format. See
           the ExportWebMap JSON specification for more
           information. The ArcGIS Web APIs (JavaScript, Flex, and
           Silverlight) allow developers to easily get this JSON string from
           a  web application.
           
           The layer definition is a subset of the webmap_json used in the ConvertWebMapToMapDocument function. You don't need to create the web map JSON; the APIs take care of it
           for you. However, you need to extract the layer definition from the 
           full webmap_json ."""
        if not isinstance(json_layer_definition, (unicode, str)):
            import json
            json_layer_definition = json.dumps(json_layer_definition)
        return convertArcObjectToPythonObject(self._arc_object.updateLayerFromJSON(*gp_fixargs((json_layer_definition,), True)))
...
class TableView(mixins.TableViewMixin,_BaseArcObject):
    ...
    def getSelectionSet(self):
        """TableView.getSelectionSet()
           
           Returns a table's selection as a Python set of Object IDs."""
        return convertArcObjectToPythonObject(self._arc_object.getSelectionSet(*gp_fixargs((), True)))
    
    def setSelectionSet(self, method, oidList):
        """TableView.setSelectionSet(method, oidList)
