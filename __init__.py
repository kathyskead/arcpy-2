...
def GetInstallInfo(product=None):
    """GetInstallInfo()
       ...
       Returns a data dictionary that contains information on the installation
       type properties for the current version of the Esri Mapping and Charting
       solutions software installed on a particular machine."""
...
def GetLogHistory():
    """GetLogHistory()
       ...
       When log history is activated, after each geoprocessing tool is run, an
       entry with the information about the tool will be added to: The history
       log file , an Extensible Markup language (XML) file. The Geoprocessing
       history section of the metadata of the data modified or created by the
       tool."""
...
def CreateGeocodeSDDraft(loc_path, out_sddraft, service_name, server_type='ARCGIS_SERVER', connection_file_path=None, copy_data_to_server=False, folder_name=None, summary=None, tags=None, max_result_size=500, max_batch_size=1000, suggested_batch_size=1000, supported_operations=["GEOCODE","REVERSE_GEOCODE"]):
    """CreateGeocodeSDDraft(loc_path, out_sddraft, service_name, {server_type},
       ...
       A new connection file can be created using the
       CreateGISServerConnectionFile function.
...
def CreateImageSDDraft(raster_or_mosaic_layer, out_sddraft, service_name, server_type="ARCGIS_SERVER", connection_file_path="", copy_data_to_server=False, folder_name=None, summary=None, tags=None):
    """CreateImageSDDraft(raster_or_mosaic_layer, out_sddraft, service_name,
       ...
       data referenced by the mosaic dataset—the mosaic dataset itself or
       raster dataset registered with the server—then copy_data_to_server will
       always be False . Conversely, if the workspace that contains the source
       data referenced by the mosaic dataset—the mosaic dataset or raster
       dataset is not registered with the server—then copy_data_to_server will
       always be True .
...
def ListDataStoreItems(connection_file, datastore_type):
    """ListDataStoreItems(connection_file, datastore_type)
       ...
       the connection file found in your user profile directory. Alternatively,
       you can create a connection file from scratch using the function
...
def ValidateDataStoreItem(connection_file, datastore_type, connection_name):
    """ValidateDataStoreItem(connection_file, datastore_type, connection_name)
       ...
       file found in your user profile directory. Alternatively, you can create
       a connection file from scratch using the function
       CreateGISServerConnectionFile .
...
def AddDataStoreItem(connection_file, datastore_type, connection_name, server_path, client_path="", hostname=""):
    """AddDataStoreItem(connection_file, datastore_type, connection_name,
       ...
       See About registering your data with the server to learn more about when
       and why you should register your data.
       ...
       found in your user profile directory. Alternatively, you can create a
       connection file from scratch using the function
       CreateGISServerConnectionFile .
       ...
       parameters. See Database connections in ArcGIS Desktop to learn how to
       obtain this file or string.
       ...
       feature service from an unregistered data location. See Copying data to
       the server automatically when publishing to learn more.
