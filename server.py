    ...
    """CreateMapServerCache_server(input_service, service_cache_directory, tiling_scheme_type, scales_type, num_of_scales, dots_per_inch, tile_size, {predefined_tiling_scheme}, {tile_origin}, {scales;scales...}, {cache_tile_format}, {tile_compression_quality}, {storage_format})
     ...
     INPUTS:
      ...
      cache_tile_format {String}:
          ...
          * LERC—Limited Error Raster Compression (LERC) is an efficient lossy compression
          method recommended for single-band or elevation data with a large pixel depth.If
          you select LERC, specify the compression value in the Tile Compression Quality
          parameter (tile_compression_quality in Python)The compression represents the
          maximum tolerated error value that is applicable per pixel (not an average for
          the image). This value is specified in the units of the mosaic dataset. For
          example, if the error is 10 centimeters and the mosaic dataset is in meters,
          enter 0.1.LERC-based cache services will not re-sample to draw if the source
          data is not supported at a given scale or if overviews are not present. LERC-
          based cache services will also contain a properties file that lists whether or
          not the service contains elevation data.
      tile_compression_quality {Double}:
...
@gptooldoc('UpgradeMapServerCacheStorageFormat_server', None)
def UpgradeMapServerCacheStorageFormat(input_service=None, num_of_caching_service_instances=None):
    ...
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject
    try:
        retval = convertArcObjectToPythonObject(gp.UpgradeMapServerCacheStorageFormat_server(*gp_fixargs((input_service, num_of_caching_service_instances), True)))
        return retval
    except Exception, e:
        raise e
...
def ExtractData(Layers_to_Clip=None, Area_of_Interest=None, Feature_Format=None, Raster_Format=None, Spatial_Reference=None, Custom_Spatial_Reference_Folder=None, Output_Zip_File=None):
    """ExtractData_server(Layers_to_Clip;Layers_to_Clip..., Area_of_Interest, Feature_Format, Raster_Format, Spatial_Reference, {Custom_Spatial_Reference_Folder}, Output_Zip_File)
        ...
        tool is intended primarily for use as part of a geoprocessing service. When
     ...
     INPUTS:
      Layers_to_Clip (Layer):
          The layers to be clipped. Layers must be feature or raster layers in the map's
          table of contents. Layer files do not work for this parameter.
      Area_of_Interest (Feature Set):
      ...
      Feature_Format (String):
          ...
          * Bentley Microstation Design (V8) - DGN_V8 - .dgn
          ...
          DWG_R14, DWG_R2000, DWG_R2004, DWG_R2005, DWG_R2007, DWG_R2010, DXF_R14,
          DXF_R2000, DXF_R2004, DXF_R2005, DXF_R2007, and DXF_R2010.Exporting to
          nondefault formats is supported using the Quick Export tool and
...
def ExtractDataTask(Layers_to_Clip=None, Area_of_Interest=None, Feature_Format=None, Raster_Format=None, Output_Zip_File=None):
    """ExtractDataTask_server(Layers_to_Clip;Layers_to_Clip..., Area_of_Interest, Feature_Format, Raster_Format, Output_Zip_File)
        ...
        formats and spatial reference, then returns all the data in a .zip file.This
        tool is intended primarily for use as part of a geoprocessing service. When
        using this tool as part of a geoprocessing service, copy the tool into a custom
        toolbox, edit the model, and reconfigure it as necessary. For step-by-step
        instructions on how to make, use, and configure a geoprocessing service using
...
# Publishing toolset
@gptooldoc('MXDToWebMap_server', None)
def MXDToWebMap(Input_map_document=None, Web_map_name=None, Summary=None, Tags=None, Layers_to_be_published=None, Basemap=None, Online_folder_name=None):
    """MXDToWebMap_server(Input_map_document, Web_map_name, Summary, Tags, {Layers_to_be_published;Layers_to_be_published...}, Basemap, {Online_folder_name})

        Creates a web map within a portal (like Portal for ArcGIS or your local portal)
        from an MXD (map document). Requires an account that can authenticate with
        ArcGIS.com or a local portal to publish services and create the web map.

     INPUTS:
      Input_map_document (ArcMap Document):
          The input map document of layers to convert to a web map in the portal.
      Web_map_name (String):
          The name of the output web map to be created.
      Summary (String):
          The summary for the web map. The summary will provide information about the web
          map itself when found through a search.
      Tags (String):
          Identifying tags for the web map. The tags will help make your web map more
          discoverable through searches. Enter multiple tags by using a comma to separate
          terms.
      Layers_to_be_published {String}:
          The layers to be included in the output web map. By default, the tool tries to
          convert all layers found inside the map document. Invalid layer types (such as
          service layers, for example) will be excluded from the web map even if it was
          provided as input. An empty list will also attempt to create a web map with all
          layers from the map document.
      Basemap (String):
          The Esri basemap to be used in the web map.
      Online_folder_name {String}:
          The portal folder in which to create the web map. If the folder does not already
          exist, it will be created."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject
    try:
        retval = convertArcObjectToPythonObject(gp.MXDToWebMap_server(*gp_fixargs((Input_map_document, Web_map_name, Summary, Tags, Layers_to_be_published, Basemap, Online_folder_name), True)))
        return retval
    except Exception, e:
        raise e
...
def SignInToPortal(username=None, password=None, portal_url=None):
    """SignInToPortal_server(username, password, portal_url)
        ...
        own computer networks. It is called Portal for ArcGIS.Signing in to ArcGIS
        Online or Portal for ArcGIS through ArcGIS for Desktop has
        changed in the ArcGIS for Desktop 10.2 release. You may receive Warning 1561
        when executing this tool. If you receive this message you cannot use the Sign In
        To Portal tool to authenticate, but must sign in to ArcGIS Online or Portal by
        clicking File Sign In. See Signing in to ArcGIS Online in ArcGIS for Desktop
        applications for more information.

     INPUTS:
      username (String):
          Portal for ArcGIS username. This parameter is optional for portals with
          Integrated Windows Authentication.
      password (Encrypted String):
          Portal for ArcGIS password. This parameter is optional for portals with
          Integrated Windows Authentication.
      portal_url (String):
          The Sign In To Portal tool reads the URL set in Desktop Administrator. Any user
          entered value is ignored. The default value is URL for the ArcGIS portal
          currently chosen by the user in Desktop Administrator."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject
    try:
